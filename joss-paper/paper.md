...
Next to the practical value of `DataBallPy`, as it provides low-code access to scientific features (see the Features section below), `DataBallPy` also provides as an educational tool. Often, open-source Python packages provide information on how to get working code, but not on how the code works. `DataBallPy` explicitly goes a step further by elaborately explaining step by step how scientific papers are transformed into code, often referring to specific mathematical formulas as presented in the paper. This explanation is crucial since it (1) allows researchers and practitioners to better understand the strengths and weaknesses of features, and (2) teaches users how to transform scientific papers into modular, Pythonic code. Both these characteristics provide users of `DataBallPy` a better understanding of their own analysis.
...
`DataBallPy` uses a soccer-specific implementation of the Needleman-Wunsch algorithm to synchronise the event and tracking data, which is more elaborately described in [@Oonk2025]. The game can be synchronised using the following code
```python
