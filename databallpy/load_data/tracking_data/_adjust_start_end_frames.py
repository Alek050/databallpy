...
def _adjust_start_end_frames(td:pd.DataFrame, metadata:Metadata) -> Metadata:
    ...
    for i, period_row in metadata.periods_frames.iterrows():
        ...
        # for the first frame, ball should be in the centre and players should be on opposing halves
        if len(first_frame) == 1:
            ...
            players_mask = (away_players_x < 0 and home_players_x > 0) or (away_players_x > 0 and home_players_x < 0)
