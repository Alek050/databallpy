...
def align_player_ids_jersey(metadata1: Metadata, metadata2: Metadata) -> Metadata:
    ...
    metadata1.home_players.insert(
        0, 
        "id",  
        metadata1.home_players["shirt_num"].map(metadata2.home_players.set_index("shirt_num")["id"])
    )
    metadata1.away_players.insert(
        0, 
        "id",  
        metadata1.away_players["shirt_num"].map(metadata2.away_players.set_index("shirt_num")["id"])
    )
    return metadata1
...
def align_player_ids_name_similarity(metadata1: Metadata, metadata2: Metadata) -> Metadata:
