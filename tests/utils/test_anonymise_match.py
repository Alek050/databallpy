...
class TestAnonymiseMatch(unittest.TestCase):
    def setUp(self):
        ...
        self.match.home_players.loc[
            self.match.home_players["full_name"] == "Jan Boskamp", "shirt_num"
        ] = 34
        self.match.away_players.loc[
            self.match.away_players["full_name"] == "Pepijn Blok", "shirt_num"
        ] = 17
    ...
    def test_rename_tracking_data_columns(self):
        ...
        res_td = rename_tracking_data_columns(
            self.match.tracking_data.copy(), jersey_map, "home"
        )
        for exp_col in ["home_1_x", "home_1_y", "home_2_x", "home_2_y"]:
            assert exp_col in res_td.columns

        np.testing.assert_allclose(
            res_td["home_1_x"].values, self.match.tracking_data["home_34_x"].values
        )
        np.testing.assert_allclose(
            res_td["home_2_y"].values, self.match.tracking_data["home_1_y"].values
        )
    ...
    def test_anonymise_teams(self):
        ...
        for val in [
            res_match.home_team_id,
            res_match.away_team_id,
            res_match.home_team_id,
            res_match.away_team_id,
        ]:
            assert val[:2] == "T-"
    ...
    def test_anonymise_datetime(self):
        ...
        dt_diff = (
            input_match.tracking_data["datetime"].iloc[0]
            - input_match.event_data["datetime"].iloc[0]
        )
        input_match.event_data["datetime"] += dt_diff + pd.to_timedelta(
            5, unit="seconds"
        )
        ed_dt_diff = (
            input_match.event_data["datetime"].iloc[0]
            - input_match.periods["start_datetime_ed"].iloc[0]
        )
        ...
        assert (
            res_match.tracking_data["datetime"].iloc[0]
            - res_match.tracking_data["datetime"].iloc[-1]
        ) == (
            input_match.tracking_data["datetime"].iloc[0]
            - input_match.tracking_data["datetime"].iloc[-1]
        )
        assert res_match.periods["start_datetime_td"].iloc[0] == base_time
        np.testing.assert_almost_equal(
            res_match.tracking_data["frame"],
            np.arange(len(res_match.tracking_data)) + 1,
        )
        assert res_match.periods["start_frame"].iloc[0] == 1

        np.testing.assert_almost_equal(
            res_match.away_players["start_frame"].values, [1, 1]
        )
        np.testing.assert_almost_equal(
            res_match.home_players["end_frame"].values, [5, 3]
        )

        assert res_match.event_data["datetime"].iloc[0] == base_time + pd.to_timedelta(
            5, unit="seconds"
        )
        assert (
            res_match.event_data["datetime"].iloc[0]
            - res_match.event_data["datetime"].iloc[-1]
        ) == (
            input_match.event_data["datetime"].iloc[0]
            - input_match.event_data["datetime"].iloc[-1]
        )
        assert res_match.event_data.loc[11, "tracking_frame"] == 4
        assert res_match.periods["start_datetime_ed"].iloc[
            0
        ...
        assert res_match.dribbles_df["datetime"].iloc[0] == base_time + pd.to_timedelta(
            5, unit="seconds"
        )
        assert res_match.shots_df["datetime"].iloc[0] == base_time + pd.to_timedelta(
            5, unit="seconds"
        )
        assert res_match.passes_df["datetime"].iloc[0] == base_time + pd.to_timedelta(
            5, unit="seconds"
        )
    ...
    def test_anonymise_datetime_no_tracking_data(self):
        ...
        input_match.periods.loc[0, "start_datetime_ed"] = input_match.event_data[
            "datetime"
        ...
        input_match._passes_df = pd.DataFrame(
            {"datetime": [ed_dt + pd.Timedelta(15, unit="seconds")]}
        )
        input_match._dribbles_df = pd.DataFrame(
            {"datetime": [ed_dt + pd.Timedelta(15, unit="seconds")]}
        )
        input_match._shots_df = pd.DataFrame(
            {"datetime": [ed_dt + pd.Timedelta(15, unit="seconds")]}
        )
        ...
        assert res_match.periods["start_datetime_ed"].iloc[
            0
        ] == base_time - pd.Timedelta(15, unit="seconds")
