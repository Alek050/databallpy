...
class TestAddBallDataToDict(unittest.TestCase):
    def setUp(self):
        ...
        self.expected_dict = {
            ...
            "ball_z": [7.0, 15.0, 7.0],
            "team_possession": ["away", "home", "away"],
    ...
    def test_add_ball_data_to_dict(self):
        data = {
            ...
            "ball_y": [np.nan] * 3,
            "ball_z": [np.nan] * 3,
            "team_possession": [np.nan] * 3,
            "ball_status": [np.nan] * 3,
        }

        for i, input in enumerate(self.ball_info):
            data["frame"][i] = int(i)
            ball_x, ball_y, ball_z, team_possession, ball_status = input
            data = _add_ball_data_to_dict(
                ball_x, ball_y, ball_z, team_possession, ball_status.lower(), data, i
            )
