...
from tests.expected_outcomes import MD_INSTAT, ED_INSTAT
...
class TestInstat(unittest.TestCase):
    ...
    def test_load_instat_event_data(self):
        ...
        event_data, metadata = load_instat_event_data(self.instat_event_data_loc, self.instat_metadata_loc)
    ...
    def test_load_metadata(self):
        ...
        expected_metadata = MD_INSTAT.copy()
        expected_metadata.home_players = pd.DataFrame(columns=["id", "full_name", "shirt_num"])
        expected_metadata.away_players = pd.DataFrame(columns=["id", "full_name", "shirt_num"])
    ...
    def test_update_metadata(self):
        ...
        input_metadata.home_players = pd.DataFrame(columns=["id", "full_name", "shirt_num"])
        input_metadata.away_players = pd.DataFrame(columns=["id", "full_name", "shirt_num"])
        input_metadata.home_formation = ""
        input_metadata.away_formation = ""
        expected_metadata = MD_INSTAT
        assert _update_metadata(input_metadata, self.instat_event_data_loc) == expected_metadata

    def test_load_event_data(self):
        event_data, pitch_dimensions = _load_event_data(self.instat_event_data_loc, MD_INSTAT)
