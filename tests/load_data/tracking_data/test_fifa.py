...
from databallpy.load_data.tracking_data.fifa import (
    _get_metadata,
    _get_td_channels,
    _get_player_data,
    _get_tracking_data,
    load_fifa_tracking_data,
)
from databallpy.load_data.tracking_data._normalize_playing_direction_tracking import _normalize_playing_direction_tracking
from tests.expected_outcomes import MD_FIFA, TD_FIFA


class TestFifa(unittest.TestCase):
    def setUp(self):
        self.tracking_data_loc = r"tests/test_data/fifa_td_test.txt"
        self.metadata_loc = r"tests/test_data/fifa_metadata_test.xml"
    ...
    def test_get_td_channels(self):
        channels = _get_td_channels(self.metadata_loc, MD_FIFA)
        assert channels == self.expected_channels

    def test_get_tracking_data(self):
        tracking_data = _get_tracking_data(self.tracking_data_loc, self.expected_channels, [100.0, 50.0])
        expected_td = TD_FIFA.drop(["matchtime_td", "period"], axis=1)
        pd.testing.assert_frame_equal(tracking_data, expected_td)

    def test_load_tracking_data(self):
        tracking_data, metadata = load_fifa_tracking_data(
            self.tracking_data_loc, self.metadata_loc, verbose=False
        )
        expected_tracking_data = TD_FIFA.iloc[1:,:].reset_index(drop=True)
        expected_tracking_data = _normalize_playing_direction_tracking(
            expected_tracking_data, MD_FIFA.periods_frames
        )
        assert metadata == MD_FIFA
